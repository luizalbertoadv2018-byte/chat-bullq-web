...
/**
 * Tabela de contatos (CRM). Componente compartilhado entre a página standalone
 * /contacts (estilo LiderHub) e a aba Configurações → Contatos.
 * `fullWidth` solta a largura máxima para a página standalone ocupar a tela.
 */
export function ContactsTable({ fullWidth = false }: { fullWidth?: boolean }) {
  ...
  const widthCls = fullWidth ? 'w-full' : 'mx-auto w-full max-w-5xl';

  return (
    <div className="flex h-full flex-col min-h-0 min-w-0 p-6">
      <div className={`${widthCls} shrink-0`}>
      ...
      <div className={`${widthCls} mt-4 flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900`}>
